...
- 两平台各 103 个文件，ZIP CRC、逐文件 SHA-256、生产公钥一致性、正式码／私钥／配对 Token 排除、相对链接和两个公开案例检查均通过。
- Release 已发布，四个附件通过无凭据官方 API 下载回验；当时 Release 直接下载 URL 返回 404，因此在原仓库 downloads/ 增加同字节镜像，首页使用镜像链接。
