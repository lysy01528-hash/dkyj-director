## 2026-09-16 · 固定白模与单描述建模

- 默认固定球头＋长方体身体，两块纯色网格，不含四肢和人物细节；增加标准人物生成 MCP 工具与完成检查。
- 欢迎页合并成一个描述框，直接“开始建模”；旧草稿合并，新任务不会继承详细模式。
- 仅显式详细开关放开细节；普通描述不再触发额外确认。任务仍由连接的 Agent 执行。
- 本次仅更新本地源码和测试包，未上传 GitHub。

## 2026-09-16 · Capture session / 本地候选测试版

- 固定运镜机位，重复录制复用当前轨迹；保留旧用户相机和历史 Take。
- 分离手机与手柄输入，增加执行回执和重试去重；修复共享 Action 重复清理。
- 手机独立横屏双摇杆，常用按钮精简，低频操作折叠；Safari 全屏能力不足时保留可用触控。
- 导出使用已录轨迹快照，文件名防同秒覆盖；修复空间路径末帧时间。
- 已做隔离回归与真实三种导出验证，详情见 [验证记录](fixes/capture-session.md)。本次未上传 GitHub。

# 开发记录

## 2026-09-14 · 场景与人物简化／细节建模（本地）

欢迎页加入中性灰建模档位，默认简化，同时约束场景和人物。提交、草稿恢复、任务卡片与补充回复携带档位；旧简报默认简化。Agent 收到结构化场景、人物、运动、保留与验证要求；描述提到细节而未明确选择时，先询问，不创建或切换项目。后端阻止未确认任务直接进入制作或完成状态。

简化档仅保留运镜所需体块、通道、基础人物与可编辑动作。细节档按需增加，不默认添加模拟、装饰和额外动态。原项目不自动减面或删除物体。此功能是 Agent 建模约束，不声称已对现有精细模型完成自动优化。

检查：7 项新增持久化／确认／MCP 前置阻断测试、2 项原简报测试、前端默认／草稿／提交／回复测试通过；原网页控制和 45 项手柄合成测试通过。Safari 隔离界面实际确认默认简化、切换细节与说明更新；本地导演台服务当时未运行，未在用户工程中启动 Agent 建模。更新的本地 Pro 包继续免激活。没有上传 GitHub。
...
手柄模式与手机体感互斥；在同一网页内，键盘、拖动或屏幕按钮接管时暂停手柄输入。同一时刻请只在一台设备启用控制，当时尚无跨设备控制锁；2026-09-16 已由独占会话替代此行为。网页失焦、切到后台或手柄断连时暂停连续输入并回零；重连后先回中，再重新点击「手柄运镜」启用，不积压旧的移动或转向指令。界面「机位 Z」显示世界坐标 Z，不是离地高度。
